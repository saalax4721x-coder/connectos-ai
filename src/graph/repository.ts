...
export class InMemoryGraph implements GraphRepository {
  ...
  async findPath(from:string,to:string,maxDepth:number){
    ...
    const out:GraphEdge[][]=[];
    const walk=(id:string,path:GraphEdge[],seen:Set<string>)=>{
      if(path.length>maxDepth||out.length>=25)return;
      if(id===to){out.push(path);return;}
      for(const e of this.edges.values()){
        if(!e.active||e.from!==id||seen.has(e.to))continue;
        walk(e.to,[...path,e],new Set(seen).add(e.to));
      }
    };
  ...
  async query(input:GraphQuery):Promise<GraphResult>{
    ...
    const edgeTypes=new Set(q.edgeTypes??[]);
    const minConfidence=q.minConfidence??0;
    const minStrength=q.minStrength??0;
    const eligible=(e:GraphEdge)=>e.active&&e.confidence>=minConfidence&&e.strength>=minStrength&&(edgeTypes.size===0||edgeTypes.has(e.type));
    const edges=[...this.edges.values()].filter(eligible).filter(e=>q.includeStale||decayedStrength(e)>=minStrength);
    const paths:Array<{nodes:string[];edges:GraphEdge[];score:number}>=[];
    const walk=(id:string,nodes:string[],path:GraphEdge[])=>{
      if(paths.length>=q.limit||path.length>=q.depth)return;
      for(const e of edges.filter(x=>x.from===id)){
        if(nodes.includes(e.to))continue;
        const next=[...path,e];
        const nextNodes=[...nodes,e.to];
        if(!q.target||e.to===q.target) paths.push({nodes:nextNodes,edges:next,score:next.reduce((s,x)=>s*x.confidence*x.strength,1)});
        if(!q.target||e.to!==q.target) walk(e.to,nextNodes,next);
        if(paths.length>=q.limit)return;
      }
    };
    if(this.nodes.has(q.start)) walk(q.start,[q.start],[]);
    paths.sort((a,b)=>b.score-a.score);
    const chosenEdges=new Map<string,GraphEdge>();
    for(const path of paths)for(const edge of path.edges)chosenEdges.set(edge.id,edge);
  ...
  private upsertNodeSync(node:GraphNode){
    node.kind;
  }
  private upsertEdgeSync(edge:GraphEdge){
    validateEdge(edge);
    if(!this.nodes.has(edge.from)||!this.nodes.has(edge.to)) throw new Error('graph edge endpoints must exist');
