  ...
  return (
    <main style={{minHeight:'100vh',background:'#0a0d12',color:'#f5f7fa',fontFamily:'Inter,system-ui,sans-serif',padding:'48px 24px'}}>
      <section style={{maxWidth:980,margin:'0 auto'}}>
        ...
        <form onSubmit={(event)=>{event.preventDefault();setSubmitted(goal.trim());}} style={{display:'flex',gap:12,marginTop:32}}>
          <input value={goal} onChange={(event)=>setGoal(event.target.value)} placeholder="Find three serious distributors for my perfume brand in Kenya" style={{flex:1,padding:'18px 20px',borderRadius:14,border:'1px solid #2a303a',background:'#11161d',color:'inherit',fontSize:16}} />
          <button disabled={!goal.trim()} style={{padding:'0 24px',borderRadius:14,border:0,background:'#f5f7fa',color:'#0a0d12',fontWeight:700}}>Plan</button>
        </form>
        ...
        {result && <section style={{marginTop:36,display:'grid',gap:16}}>
          <div style={{padding:20,border:'1px solid #252b35',borderRadius:16,background:'#0f141b'}}>
            ...
            <strong style={{display:'block',marginTop:8,fontSize:20}}>{result.status === 'WAITING_APPROVAL' ? 'Waiting for your approval' : 'Ready to proceed'}</strong>
