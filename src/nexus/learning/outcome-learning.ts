...
export function learnFromOutcomes(outcomes: NexusOutcome[], planId: string): LearnedSignal {
  ...
  return {sampleCount: relevant.length, successRate: success / relevant.length, correctionRate: corrected / relevant.length, lastOutcome: latest.kind};
}
